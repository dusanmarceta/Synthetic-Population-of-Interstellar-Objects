...
def synthetic_population_stationary(T, rm, n0, v_min, v_max, 
                         u_Sun, v_Sun, w_Sun, 
                         sigma_vx, sigma_vy, sigma_vz, 
                         vd, va, R_reff,
...
#    b_arr = np.linspace(-np.pi/2, np.pi/2, angle_resolution//2)
    ...
    # gustina verovatnoće (tvoj postojeći p_v_l_b)
    p_vlb_f = p_v_l_b(
        v_f, l_f, b_f,
        sigma_vx, -u_Sun,
        sigma_vy, -v_Sun,
...
#    print("Integral p_v_l_b (v,l,b):", np.sum(p_vlb_f * dv * db * dl))
#    print("Total flux (objekata/s):", total_flux)
...
#    print('suma brzine', np.sum(p_vlb * dv * db * dl))
...
# =============================================================================
        ...
        for i in tqdm(range(len(v_sample)), desc='6/6: Conversion to orbital elements dodatnih'):
            ...
            # argument of perihelion
            argument_add[i] = np.arccos(np.linalg.linalg.dot(nn, e_vector) / np.linalg.norm(nn) / np.linalg.norm(e_vector))
...
#    print(np.shape(q_add))
            
    
#    print(total_number, len(M_add))
#        
#      
    if len(d)>1:
        return(q_s/au, e_s, f_s, inc_s, node_s, argument_s, D_s)
        ...
        return(np.concatenate([q_s, q_add])/au, np.concatenate([e_s, e_add]),
               ...
               q_add, e_add, f_add, inc_add, node_add, argument_add, t_add, r_add)
