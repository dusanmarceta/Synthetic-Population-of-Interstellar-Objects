...
#Gaia.TAP_URL = "https://gaia.aip.de/tap"  # Gaia@AIP
...
def p_vx_vy_vz1(U, V, W, p_min):
#    Gaia.login(credentials_file="login.txt") # a file containing your username on line 1 and your password on line 2
#
#    T_min = 0
#    T_max = 1000000
#
...
#    m = [-100000, -1]
    ...
    WHERE 
        ...
        AND g.teff_gspphot IS NOT NULL
    	AND g.teff_gspphot BETWEEN {T_min} AND {T_max}
        AND g.mh_gspphot > {m[0]}
        AND g.mh_gspphot < {m[1]}
        AND g.parallax_over_error > 10
        AND g.visibility_periods_used > 8
        AND g.astrometric_n_obs_al > 100
    """
    ...
    vx_eq = v_r*np.cos(alpha)*np.cos(delta) - v_a*np.sin(alpha) - v_d*np.cos(alpha)*np.sin(delta)
    vy_eq = v_r*np.sin(alpha)*np.cos(delta) + v_a*np.cos(alpha) - v_d*np.sin(alpha)*np.sin(delta)
    vz_eq = v_r*np.sin(delta)               + 0.0               + v_d*np.cos(delta)

    alpha_Omega = np.deg2rad(282.86)
    l_Omega = np.deg2rad(32.93)
    i = np.deg2rad(62.87)

    u_gal = (
        ( np.cos(alpha_Omega)*np.cos(l_Omega) + np.cos(i)*np.sin(alpha_Omega)*np.sin(l_Omega) ) * vx_eq +
        ( np.sin(alpha_Omega)*np.cos(l_Omega) - np.cos(i)*np.cos(alpha_Omega)*np.sin(l_Omega) ) * vy_eq +
        ( -np.sin(i)*np.sin(l_Omega)                                                          ) * vz_eq 
        )

    v_gal = (
        ( np.cos(alpha_Omega)*np.sin(l_Omega) - np.cos(i)*np.sin(alpha_Omega)*np.cos(l_Omega) ) * vx_eq +
        ( np.sin(alpha_Omega)*np.sin(l_Omega) + np.cos(i)*np.cos(alpha_Omega)*np.cos(l_Omega) ) * vy_eq +
        ( np.sin(i)*np.cos(l_Omega)                                                           ) * vz_eq 
        )

    w_gal = (
        ( np.sin(i)*np.sin(alpha_Omega)  ) * vx_eq +
        ( -np.sin(i)*np.cos(alpha_Omega) ) * vy_eq +
        ( np.cos(i)                      ) * vz_eq 
        )
...
#    
    ...
    original_data = [u_gal, v_gal, w_gal]
    kde = stats.gaussian_kde(original_data)



    positions = np.vstack([U.ravel(), V.ravel(), W.ravel()])
    ...
    print('stiglo D')
    pdf_3d = density_values.reshape(U.shape)
    print('stiglo E')

    return pdf_3d

def p_v_l_b1(v, l, b, p_min):
    vx=-v*np.cos(b)*np.cos(l)
    vy=-v*np.cos(b)*np.sin(l)
    vz=-v*np.sin(b)

    return p_vx_vy_vz1(vx, vy, vz, p_min)*(vx**2+vy**2+vz**2)*np.cos(b)


def synthetic_population1(rm, n0, R_reff, p_min, v_mag_min, v_mag_max,
    ...
    v_arr=np.linspace(v_mag_min, v_mag_max,speed_resolution)
...
# ============================================================================= 
        ...
        for i in range(len(new_sizes)):
            ...
            for j in range(new_sizes[i]):
                v_try=0
                while v_try<v_mag_min or v_try>v_mag_max: # ensuring that the resulting value falls in allowed range of interstellar velocities
...
# =============================================================================
        ...
        for i in range(len(new_sizes)):
            ...
            p_lb__vs=p_v_l_b1(vs_vec, l_vec, b_vec, p_min)
...
# =============================================================================
        ...
        for i in range(len(new_sizes)):
            ...
            print(np.shape(vs_vec))
            print(np.shape(ls_vec))
            print(np.shape(b_vec))
            p_b__vs__ls=p_v_l_b1(vs_vec, ls_vec, b_vec, p_min)
